...
class StepService {
  ...
  late final Future<void> _healthConfigured;
  ...
  StepService(this.prefs, {ActivityDetectionService? activityDetection})
    ...
    _healthConfigured = _health.configure();
  ...
  Future<bool> _ensureHealthConnectReady() async {
    ...
    try {
      ...
      if (!isAvailable) {
        ...
        debugPrint('Health Connect unavailable: $status');
      }
      return isAvailable;
    } catch (e) {
  ...
  Future<void> syncWithHealth() async {
    ...
    if (!(await _ensureHealthConnectReady())) {
      return;
    }
